        ...
        <div className="w-6 h-5 flex flex-col justify-between">
          <span
            className={`block h-0.5 w-full bg-foreground rounded-full transition-all duration-300 ease-in-out ${
              isOpen ? 'rotate-45 translate-y-2.25' : 'rotate-0 translate-y-0'
            }`}
          />
          <span
            className={`block h-0.5 w-full bg-foreground rounded-full transition-all duration-300 ease-in-out ${
              ...
              {menuItems.map((item, index) => (
                <li key={`burger-${String(index)}`}>
                  {item.external ? (
                    <ExternalLink
                      href={item.href}
                      className="flex items-center gap-3 px-4 py-3 0 hover:bg-(--eva-gold)/80 hover:text-(--eva-black) rounded-lg transition-colors duration-200 group"
                    >
                      <>
                        {item.icon && (
                          <span className="text-gray-500 group-hover:text-(--eva-gold) transition-colors">
                            <item.icon />
                          </span>
                        )}
